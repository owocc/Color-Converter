...
const ColorTooltip: React.FC<{
  ...
  const showComparison =
    compareEnabled &&
    data.original &&
    data.original.color !== data.converted.color;
...
const ColorPreviewDrawer: React.FC<ColorPreviewDrawerProps> = ({
  isOpen,
  onClose,
  data,
  compareEnabled,
        ...
        <div className="relative">
          ...
          <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-[#C8C5CA]">
            <svg
              ...
              viewBox="0 0 20 20"
            >
              <path d="M5.516 7.548c.436-.446 1.144-.446 1.58 0L10 10.405l2.904-2.857c.436-.446 1.144-.446 1.58 0 .436.446.436 1.17 0 1.616l-3.7 3.64c-.436.446-1.144.446-1.58 0l-3.7-3.64c-.436-.446-.436-1.17 0-1.616z" />
            </svg>
          </div>
        </div>
      </div>
          ...
          >
            ...
            >
              <path
                ...
                strokeWidth={2}
                d="M9 17V7m5 10V7M5 17h14a2 2 0 002-2V9a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2z"
              ></path>
            </svg>
          </button>
          <button
          ...
          >
            ...
            >
              <path
                ...
                strokeLinejoin="round"
                strokeWidth={2}
                d="M3 17h18a2 2 0 002-2V9a2 2 0 00-2-2H3a2 2 0 00-2 2v6a2 2 0 002 2z"
              ></path>
            </svg>
          </button>
        </div>
...
const ColorConverter: React.FC = () => {
  ...
  const [useCssSyntax, setUseCssSyntax] = useLocalStorage<boolean>(
    "settings:useCssSyntax",
    true,
  );

  const [tooltipData, setTooltipData] = useState<TooltipData | null>(null);
  const [isSettingsOpen, setIsSettingsOpen] = useState(false);
  const [previewBg, setPreviewBg] = useLocalStorage<string>(
    "settings:previewBg",
    "#49454F",
  );
  const [showColorPreviews, setShowColorPreviews] = useLocalStorage<boolean>(
    "settings:showColorPreviews",
    true,
  );
  const [compareOnPreview, setCompareOnPreview] = useLocalStorage<boolean>(
    "settings:compareOnPreview",
    true,
  );
  ...
  const [showRayButton, setShowRayButton] = useLocalStorage<boolean>(
    "settings:showRayButton",
    true,
  );
  const [syncScroll, setSyncScroll] = useLocalStorage<boolean>(
    "settings:syncScroll",
    true,
  );
  ...
  const [activeSingleView, setActiveSingleView] = useState<"input" | "output">(
    "input",
  );
  ...
  const inputColors = useMemo(
    () => inputText.match(COLOR_REGEX) || [],
    [inputText],
  );
  const outputColors = useMemo(
    () => outputText.match(COLOR_REGEX) || [],
    [outputText],
  );
  const debouncedInputText = useDebouncedValue(inputText, 200);

  useEffect(() => {
    const convertedText = convertCssColors(debouncedInputText, {
      format: outputFormat,
      useCssSyntax,
    });
  ...
  const handleInputColorHover = useCallback(
    (color: string, index: number, event: React.MouseEvent) => {
  ...
  const handleOutputColorHover = useCallback(
    (convertedColor: string, index: number, event: React.MouseEvent) => {
      if (!showColorPreviews) return;
  ...
  const handleInputColorClick = useCallback(
  ...
  const handleOutputColorClick = useCallback(
    (convertedColor: string, index: number) => {
      if (!showColorPreviews) return;
      const originalColor = inputColors[index];
      setTooltipData({
        original: originalColor ? { color: originalColor } : undefined,
  ...
  const handleInputScroll = useCallback((scrollTop: number) => {
    if (syncingRef.current) return;
    syncingRef.current = true;
    outputEditorRef.current?.scrollTo(scrollTop);
  ...
  const handleOutputScroll = useCallback((scrollTop: number) => {
    if (syncingRef.current) return;
    syncingRef.current = true;
    inputEditorRef.current?.scrollTo(scrollTop);
    requestAnimationFrame(() => {
      syncingRef.current = false;
    });
  }, []);
  ...
  const settingsProps = {
    ...
    showRayButton,
    onShowRayButtonChange: setShowRayButton,
    syncScroll,
    onSyncScrollChange: setSyncScroll,
    currentBg: previewBg,
    onBgChange: (color: string) => {
      setPreviewBg(color);
      setIsSettingsOpen(false);
    },
    viewMode,
  ...
  return (
    <div className="flex flex-col gap-12">
      ...
      <div className="flex flex-col">
        ...
        <div
          className={`grid ${viewMode === "dual" ? "md:grid-cols-2" : "grid-cols-1"} grid-cols-1 gap-x-6 gap-y-8`}
        >
          <div
            role="tabpanel"
            id="panel-input"
          ...
          >
            ...
            >
              <CodeEditor
                ...
                onScroll={syncScroll ? handleInputScroll : undefined}
              />
            </div>
          </div>
      ...
      {!isMobile && showColorPreviews && tooltipData && (
        <ColorTooltip data={tooltipData} compareEnabled={compareOnPreview} />
      )}
      {isMobile && (
        <ColorPreviewDrawer
          isOpen={isDrawerOpen}
          ...
          data={tooltipData}
          compareEnabled={compareOnPreview}
        />
      )}
      <SettingsDrawer
